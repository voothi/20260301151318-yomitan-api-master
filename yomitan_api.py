...
def error_log(message: str, error: str = "") -> None:
    try:
        ...
        with open(os.path.join(script_path, "error.log"), "a", encoding = "utf8") as log_file:
    ...
    def do_POST(self) -> None:
        try:
            ...
            if yomitan_response and "responseStatusCode" in yomitan_response:
                ...
                send_response(self, 500, "application/json", json.dumps({"error": "Communication error with extension"}))
        except Exception:
            error_log(traceback.format_exc())
            send_response(self, 500, "application/json", json.dumps({"error": "Internal API Error"}))
